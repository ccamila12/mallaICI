...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function actualizarDisponibilidad() {
        document.querySelectorAll(".ramo").forEach(ramo => {
            ...
            const puedeCursar = prereq.every(p => completados[p]);

            if (!completados[nombre]) {
                ramo.classList.toggle("bloqueado", !puedeCursar);
            }
        });
    }
    ...
    malla.forEach(sem => {
        ...
        sem.asignaturas.forEach(asig => {
            ...
            ramo.className = "ramo";
            ramo.dataset.nombre = asig.nombre;
            ramo.dataset.prereq = JSON.stringify(asig.prereq || []);
            ...
            ramo.addEventListener("click", () => {
                const prereq = JSON.parse(ramo.dataset.prereq || "[]");
                const completados = JSON.parse(localStorage.getItem("ramosCompletados") || "{}");
                if (!prereq.every(p => completados[p])) return;
